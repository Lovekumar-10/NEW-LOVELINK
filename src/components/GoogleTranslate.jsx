...
const GoogleTranslate = () => {
  ...
  // Helper function to set the Google Translate Cookie
  ...
  useEffect(() => {
    ...
    // 3. Check for saved language on load and force it
    const savedLang = localStorage.getItem("preferredLanguage");
    if (savedLang && savedLang !== "en") {
      setTranslateCookie(savedLang);
    }

    // 4. MutationObserver to clean Google's UI
    const observer = new MutationObserver(() => {
      const banner = document.querySelector(".goog-te-banner-frame");
      if (banner) {
        banner.remove();
  ...
  const handleTranslate = (code) => {
    ...
    // Trigger Google's hidden selector
    const googleSelect = document.querySelector(".goog-te-combo");
    if (googleSelect) {
      googleSelect.value = code;
      googleSelect.dispatchEvent(new Event("change"));
    ...
    // Optional: Small delay and refresh ensures the translation engine catches up
